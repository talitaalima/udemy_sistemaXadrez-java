...
public class UI {
	...
	public static void printPartida(PartidaXadrez partidaXadrez, List<XadrezPeca> capturada) {
		...
		if (!partidaXadrez.getXequeMate()) {
			System.out.println("Aguardando jogador: " + partidaXadrez.getAtualJogador());
	...
	private static void printPecasCapturadas(List<XadrezPeca> capturada) {
		List<XadrezPeca> branco = capturada.stream().filter(x -> x.getCorPecas() == CorPecas.BRANCO).collect(Collectors.toList());
		List<XadrezPeca> preto = capturada.stream().filter(x -> x.getCorPecas() == CorPecas.PRETO).collect(Collectors.toList());
